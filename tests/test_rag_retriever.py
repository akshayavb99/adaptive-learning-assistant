...
class InsertionTests(unittest.TestCase):
    ...
    def test_heading_chunking_preserves_sections_and_splits_large_sections(self):
        content = "# Intro\nintro text\n## Details\n" + ("x" * 1200)

        chunks = RAGRetriever._chunk_document(content)

        self.assertEqual(chunks[0], "# Intro\nintro text")
        self.assertEqual(len(chunks[1]), 1000)
        self.assertLessEqual(len(chunks[2]), 1000)
        self.assertEqual(chunks[1][-200:], chunks[2][:200])
