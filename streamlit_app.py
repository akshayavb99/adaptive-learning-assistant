...
def get_runtime():
    ...
    usage_recorder = UsageRecorder(connection)
    question_bank = QuestionBankStore(connection, embedder)
    client = OpenAIRAGClient(retriever, usage_recorder=usage_recorder, question_bank=question_bank)
...
def end_test(client: OpenAIRAGClient) -> None:
...
try:
    retriever, client, startup_processed = get_runtime()
except Exception as exc:
                        ...
                        try:
                            ...
                            if is_correct:
                                st.session_state.difficulty = min(
                                    5, st.session_state.difficulty + 1
                                )
                            else:
                                st.session_state.difficulty = max(
                                    1, st.session_state.difficulty - 1
                                )
                            st.rerun()
                        except Exception as exc:
